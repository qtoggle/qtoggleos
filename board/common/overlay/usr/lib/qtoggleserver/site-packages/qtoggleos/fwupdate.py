...
class FWUpdateDriver(fwupdate.BaseDriver):
    ...
    async def _call_fwupdate(self, args: list[str]) -> str:
        if args and args[0] in ("auto", "current"):
            log = logger.debug
        else:
            log = logger.info
        
        args = ["chroot", self.HOST_ROOT, self.FWUPDATE_BIN] + args
        log("calling \"%s\"", " ".join(args[2:]))
    ...
    def _docker_compose_trigger_pull(self) -> None:
        logger.info("triggering OS compose-pull")
    ...
    def _docker_compose_pull_running(self) -> bool:
        ...
        for p in psutil.process_iter(["cmdline"]):
            try:
                cmdlines.append(" ".join(p.cmdline()))
            except Exception:
                pass

        return any(cmdline for cmdline in cmdlines if re.match("^.*docker.*compose.*pull$", cmdline))
