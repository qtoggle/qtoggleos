...
class FWUpdateDriver(fwupdate.BaseDriver):
    ...
    async def update_to_version(self, version: str) -> None:
        ...
        if target_qs_version != current_qs_version:
            ...
            _update_docker_compose_env(target_qs_version)
            if target_os_version == current_os_version:
                # If OS doesn't need update, reboot manually
                _fake_fwupdate_reboot()
                system.reboot()
    ...
    def _update_docker_compose_env(qs_version: str) -> None:
        try:
